...
void ptxJIT(CUmodule* ph_module,
            CUfunction* ph_kernel,
            CUlinkState* linker_state,
            const std::string ptx_file_path,
...
struct ptx_kernel {
  ...
  void compile(const bool quiet = false) {
    ...
    ptxJIT(&h_module_, &h_kernel_, &link_state_, kernel_path_, kernel_entry_string_, quiet);
  }
